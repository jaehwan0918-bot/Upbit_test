...
function calculate(rows){
  const a=rows.slice().reverse().map(r=>({time:new Date(r.candle_date_time_utc+"Z"),open:+r.opening_price,high:+r.high_price,low:+r.low_price,close:+r.trade_price,volume:+r.candle_acc_trade_volume}));
  ...
  return{candleTime:a[i].time.toISOString(),price:c[i],score,rsi,adx:adx[i],atrPct:atr[i]?atr[i]/c[i]*100:0,volRatio,ma20:m20[i],ma60:m60[i],ma120:m120[i],macd:macd[i],signal:sig[i],regime};
}
async function fetchOne(market,tf){const path=tf==="day"?"/v1/candles/days":`/v1/candles/minutes/${tf}`,u=new URL("https://api.upbit.com"+path);u.searchParams.set("market",market);u.searchParams.set("count","150");const r=await fetch(u.toString(),{headers:{Accept:"application/json","User-Agent":"CryptoAnalyzerV15"}});if(!r.ok)throw new Error(`Upbit ${market}/${tf}: HTTP ${r.status}`);return calculate(await r.json())}
